"""Servidor del programa principal."""
...
class Servidor:
    ...
    def __init__(self):
        ...
        self.host = HOST
        self.port = PORT
        param_1 = socket.AF_INET
        param_2 = socket.SOCK_STREAM
        self.socket_servidor = socket.socket(param_1, param_2)
    ...
    def escuchar_cliente_thread(self, client_socket):
        ...
        while True:
            try:
                ...
                response = bytearray()
                # Recibimos datos hasta que alcancemos la totalidad
                # de los datos
                # indicados en los primeros 4 bytes recibidos.
                while len(response) < response_length:
                    response += client_socket.recv(256)

                # Una vez que tenemos todos los bytes,
                # entonces ahí decodificamos
                response = response.decode()
                print(response)
                # Luego, debemos cargar lo anterior utilizando json
                decoded = json.loads(response)
                # Para evitar hacer muy largo este método,
                # el manejo del mensaje se
    ...
    def manejar_comando(self, recibido, client_socket):
        '''
        Este método toma lo recibido por el cliente correspondiente
        al socket pasado
        como argumento.
        :param recibido: diccionario de la forma:
         {"status": tipo, "data": información}
        :param client_socket:
         socket correspondiente al cliente que envió el mensaje
        :return:
        '''

        # Podemos imprimir para verificar que todo anda bien
        ...
        if recibido["status"] == "mensaje":
            ...
            self.sockets[client_socket] = recibido["data"]["usuario"]
            # print(self.sockets[client_socket])
            nuevo = recibido["data"]["usuario"]
            if nuevo not in self.clientes:
        ...
        elif recibido["status"] == "nuevo_poder":
            msj = {"status": "nuevo_poder",
                   "data": recibido["data"]}
            for skt in self.sockets.keys():
                self.send(msj, skt)

        elif recibido["status"] == "cambio_velocidad":
            msj = {"status": "cambio_velocidad",
                   "data": recibido["data"]}
            for skt in self.sockets.keys():
                self.send(msj, skt)

        elif recibido["status"] == "cambio_puntaje":
            msj = {"status": "cambio_puntaje",
                   "data": recibido["data"]}
            for skt in self.sockets.keys():
                self.send(msj, skt)

        elif recibido["status"] == "queremos_jugar":
            msj = {"status": "queremos_jugar",
                   "data": recibido["data"]}
            for skt in self.sockets.keys():
                self.send(msj, skt)
    ...
    def send(valor, socket):
        ...
        :param msg: diccionario del tipo::
